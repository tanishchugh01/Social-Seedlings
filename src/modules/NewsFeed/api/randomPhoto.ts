...
export async function randomPhotoApi({
  ...
  let resp = [{}];
  if (formatterFunction === undefined) formatterFunction = (resp: any) => resp;
  ...
  try {
    ...
    if (isCacheExist("newsFeed", urlKey) && cacheStorage && cachedResponse) {
      resp = await cachedResponse.json();
    } else {
      // page= CONSTANTS.API.UNSPLASH.page_random;
      const response = await unsplashApi.get<PhotoData[]>(urlString, {
        params: {
          count: CONSTANTS.API.UNSPLASH.PAGE_LIMIT,
          page: page,
          ...extraParams,
        },
      });
